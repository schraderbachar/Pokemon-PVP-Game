...
pokemon::pokemon(int level) : level(level)
{
    ...
    pokemon_species_index = rand() % ((sizeof(species) / sizeof(species[0])) - 1);
    s = species + pokemon_species_index;

    if (!s->levelup_moves.size())
    {
        // haven't generated a pokemon of this species before so we add its level-up moveset and save it
    ...
    for (i = 0; i < s->levelup_moves.size() && s->levelup_moves[i].level <= level; i++)
        ;
    ...
    current_hp = effective_stat[stat_hp];

    shiny = (((rand() % 5) == 0) ? true : false);
...
const int pokemon::get_move_power(int i) const
{
    if (i < 4 && move_index[i])
    {
